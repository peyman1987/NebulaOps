# NebulaOps v23.3 — Angular Micro Frontends
...
```bash
docker build -t nebulaops-mfe-<name>:23.3 .
docker run -p <port>:80 nebulaops-mfe-<name>:23.3
```
...
- **Shared JWT** — `localStorage.getItem('nebulaops.v23_3.jwt')` read by each MFE

# NebulaOps v23.4 — Angular Micro Frontends
...
```bash
docker build -t nebulaops-mfe-<name>:23.4 .
docker run -p <port>:80 nebulaops-mfe-<name>:23.4
```
...
- **Shared JWT** — `localStorage.getItem('nebulaops.v23_4.jwt')` read by each MFE

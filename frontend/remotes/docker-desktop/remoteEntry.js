...
(function(){
'use strict';
const Base = window.__NebulaBaseMfe;
class DockerMfe extends Base{
 constructor(){super();this.config={icon:'🐳',eyebrow:'Runtime · Docker',title:'Docker Desktop',subtitle:'Container, image, volume and network operations through live NebulaOps gateway endpoints. No mock rows are rendered.',tabs:[
  {id:'containers',label:'Containers',url:'/api/runtime/docker/containers',columns:['Container','State','Image / Ports','Actions']},
  {id:'images',label:'Images',url:'/api/runtime/docker/images',columns:['Image','Tag / Id','Size / Created','Actions']},
  {id:'volumes',label:'Volumes',url:'/api/runtime/docker/volumes',columns:['Volume','Driver','Mountpoint','Actions']},
  {id:'networks',label:'Networks',url:'/api/runtime/docker/networks',columns:['Network','Driver / Scope','Id','Actions']}
 ]}}
 renderRow(active,x){
  if(active.id==='containers') return `<tr><td><div class="rowTitle">${esc(x.name||short(x.id))}</div><div class="muted">${esc(short(x.id,18))}</div></td><td><span class="status ${statusClass(x.status)}">${esc(x.status||'-')}</span></td><td><div>${esc(x.image||'-')}</div><div class="muted">${esc(x.ports||x.created||'')}</div></td><td><div class="actions">${this.renderActions(active,x)}</div></td></tr>`;
  if(active.id==='images') return `<tr><td><div class="rowTitle">${esc((x.raw.Repository||x.raw.repository||x.name||'').toString())}</div><div class="muted">${esc((x.raw.Tag||x.raw.tag||x.raw.RepoTags||'').toString())}</div></td><td>${esc(short(x.id,24))}</td><td><div>${esc(x.size||'-')}</div><div class="muted">${esc(x.created)}</div></td><td><div class="actions">${this.renderActions(active,x)}</div></td></tr>`;
  if(active.id==='volumes') return `<tr><td><div class="rowTitle">${esc(x.name)}</div></td><td>${esc(x.raw.Driver||x.raw.driver||'-')}</td><td><div class="muted">${esc(x.raw.Mountpoint||x.raw.mountpoint||'-')}</div></td><td><div class="actions">${this.renderActions(active,x)}</div></td></tr>`;
  return `<tr><td><div class="rowTitle">${esc(x.name)}</div></td><td>${esc(x.raw.Driver||x.raw.driver||x.raw.Scope||'-')}</td><td>${esc(short(x.id,24))}</td><td><div class="actions">${this.renderActions(active,x)}</div></td></tr>`;
 }
 renderActions(active,x){
  const id=esc(x.id||x.name);
  if(active.id==='containers') return `<button class="btn small ok" data-action="start" data-tab="${active.id}" data-id="${id}">Start</button><button class="btn small warn" data-action="stop" data-tab="${active.id}" data-id="${id}">Stop</button><button class="btn small" data-action="restart" data-tab="${active.id}" data-id="${id}">Restart</button><button class="btn small" data-action="logs" data-tab="${active.id}" data-id="${id}">Logs</button><button class="btn small danger" data-action="remove-container" data-tab="${active.id}" data-id="${id}">Remove</button>`;
  if(active.id==='images') return `<button class="btn small danger" data-action="remove-image" data-tab="${active.id}" data-id="${id}">Remove image</button><button class="btn small warn" data-action="prune-images" data-tab="${active.id}" data-id="${id}">Prune</button><button class="btn small" data-action="raw" data-tab="${active.id}" data-id="${id}">Raw</button>`;
  if(active.id==='volumes') return `<button class="btn small danger" data-action="remove-volume" data-tab="${active.id}" data-id="${id}">Remove volume</button><button class="btn small warn" data-action="prune-volumes" data-tab="${active.id}" data-id="${id}">Prune</button>`;
  return `<button class="btn small warn" data-action="prune-networks" data-tab="${active.id}" data-id="${id}">Prune networks</button><button class="btn small" data-action="raw" data-tab="${active.id}" data-id="${id}">Raw</button>`;
 }
 async handleAction(action,id,tab){
  const item=this.findItem(tab,id)||{}; const target=encodeURIComponent((item.id||item.name||id||'').replace(/^sha256:/,''));
  const call=async(method,url,body)=>{await this.request(url,{method,body:body?JSON.stringify(body):undefined});this.toast(`${action} executed`);await this.refresh();};
  try{
    if(['start','stop','restart'].includes(action)) return call('POST',`/api/runtime/docker/containers/${target}/${action}`);
    if(action==='remove-container') return call('DELETE',`/api/runtime/docker/containers/${target}?force=false`);
    if(action==='logs'){const r=await this.request(`/api/runtime/docker/containers/${target}/logs?tail=120`);this.toast((r.logs||r.stdout||JSON.stringify(r)).toString().slice(0,1200));return;}
    if(action==='remove-image') return call('DELETE',`/api/runtime/docker/images/${target}?force=false`);
    if(action==='prune-images') return call('POST','/api/runtime/docker/images/prune',{});
    if(action==='remove-volume') return call('DELETE',`/api/runtime/docker/volumes/${encodeURIComponent(item.name||id)}`);
    if(action==='prune-volumes') return call('POST','/api/runtime/docker/volumes/prune',{});
    if(action==='prune-networks') return call('POST','/api/runtime/docker/networks/prune',{});
    return super.handleAction(action,id,tab);
  }catch(e){this.toast(`${action} failed: ${e.body?.error||e.message||e}`);}
 }
}
const esc=window.__NebulaMfeHelpers.esc, short=window.__NebulaMfeHelpers.short, statusClass=window.__NebulaMfeHelpers.statusClass;
if(!customElements.get('nebulaops-mfe-docker-desktop')) customElements.define('nebulaops-mfe-docker-desktop',DockerMfe);
})();

...
(function nebulaopsAuthBridge() {
  'use strict';
  var VERSION = 'v23.3.9-restore-ui-live-api';
  var JWT_KEY = 'nebulaops.v23_3.jwt';
  var USER_KEY = 'nebulaops.v23_3.user';
...
/* NebulaOps v23.3.0-live-real-data-control-plane · live endpoint custom element.
...
(function nebulaopsLiveRemote(){
  ...
  const TOKEN_KEY = 'nebulaops.v23_3.jwt';
  const css = `
  ...
  class LiveRemote extends HTMLElement {
    ...
    async dockerAction(action,item){ const row=meta(item); const ep=this.activeEndpoint(); const label=String(ep.label||'').toLowerCase(); const id=encodeURIComponent(row.id || row.name || ''); if(!id) return; let spec=null;
      ...
      if(!spec) return; if(['remove','kill'].includes(action) && !this.confirmTarget(`Docker ${action}`, row.name || row.id)) return; if(['stop'].includes(action) && !confirm(`Stop ${row.name || row.id}?`)) return;
    ...
    async dockerPrune(type){ if(type==='preview'){ try{ this.showDetail('Docker prune preview', await this.request('/api/runtime/docker/prune/preview')); }catch(e){ this.toast('Prune preview failed'); } return; } const map={images:'/api/runtime/docker/images/prune',volumes:'/api/runtime/docker/volumes/prune',networks:'/api/runtime/docker/networks/prune',system:'/api/runtime/docker/system/prune'}; if(!this.confirmTarget(`Docker ${type} prune`, type==='system'?'docker':'unused')) return; try{ const body=await this.request(map[type],{method:'POST',body:JSON.stringify({})}); this.showDetail(`Docker ${type} prune`,body); await this.refresh(); }catch(e){ this.toast(`${type} prune failed`); } }
    async dockerProjectAction(action,project){ if(!project) return; if(['stop','restart'].includes(action) && !this.confirmTarget(`Docker Compose project ${action}`, project)) return; try{ const method=action==='logs'?'GET':'POST'; const body=await this.request(`/api/runtime/docker/projects/${encodeURIComponent(project)}/${action}`,{method}); this.showDetail(`Docker project ${action}: ${project}`,body); if(action!=='logs') await this.refresh(); }catch(e){ this.toast(`Project ${action} failed: ${human(e.body?.error || e.message)}`); } }

    async k8sAction(action,item){ const row=meta(item); const kind=(row.kind || this.activeEndpoint().kind || '').toLowerCase(); const ns=encodeURIComponent(row.namespace || item?.metadata?.namespace || 'default'); const name=encodeURIComponent(row.name || ''); if(!name) return; let url='',method='POST',body={};
      const plural = kind.includes('daemonset') ? 'daemonsets' : kind.includes('cronjob') ? 'cronjobs' : kind.includes('job') ? 'jobs' : kind.includes('secret') ? 'secrets' : kind.includes('persistentvolumeclaim') ? 'persistentvolumeclaims' : kind.includes('storageclass') ? 'storageclasses' : kind.includes('ingress') ? 'ingresses' : kind.includes('service') ? 'services' : kind.includes('configmap') ? 'configmaps' : kind.includes('statefulset') ? 'statefulsets' : 'deployments';
      ...
      if(action==='deletePod'){ url=`/api/kubernetes/pods/${ns}/${name}`; method='DELETE'; if(!this.confirmTarget('Delete Kubernetes pod', row.name)) return; }
      ...
      if(action==='deleteJob'){ url=`/api/kubernetes/jobs/${ns}/${name}`; method='DELETE'; if(!this.confirmTarget('Delete Kubernetes job', row.name)) return; }
      ...
      if(action==='drain'){ if(!this.confirmTarget('Drain Kubernetes node', row.name)) return; url=`/api/kubernetes/nodes/${name}/drain`; }
    ...
    async yamlAction(action){ const yaml=(this.shadowRoot.getElementById('yamlEditor')?.value || '').trim(); this.state.yaml=yaml; if(!yaml){ this.toast('YAML is required'); return; } let url=action==='diff'?'/api/kubernetes/resources/diff':action==='dry-run'?'/api/kubernetes/resources/dry-run':action==='delete'?'/api/kubernetes/delete':'/api/kubernetes/apply'; if(['apply','delete'].includes(action) && !this.confirmTarget(`Kubernetes YAML ${action}`, 'manifest')) return; url=this.withCluster(url); try{ const body=await this.request(url,{method:'POST',body:JSON.stringify({yaml})}); this.showDetail(`YAML ${action} · ${this.selectedClusterLabel()}`,body); if(['apply','delete'].includes(action)) await this.refresh(); }catch(e){ this.toast(`YAML ${action} failed: ${human(e.body?.stderr || e.body?.error || e.message)}`); } }
    ...
    async helmAction(action,namespace,name){ if(!namespace||!name) return; let url=`/api/kubernetes/helm/releases/${encodeURIComponent(namespace)}/${encodeURIComponent(name)}/${action}`; let method='GET', body=undefined; if(action==='rollback'){ const revision=prompt('Rollback revision. Empty means previous revision.',''); if(revision===null) return; if(!this.confirmTarget('Rollback Helm release', name)) return; method='POST'; body=JSON.stringify({revision:Number(revision||0)}); } if(action==='uninstall'){ url=`/api/kubernetes/helm/releases/${encodeURIComponent(namespace)}/${encodeURIComponent(name)}`; method='DELETE'; if(!this.confirmTarget('Uninstall Helm release', name)) return; } try{ const result=await this.request(this.withCluster(url),{method,body}); this.showDetail(`Helm ${action}: ${namespace}/${name}`,result); if(['rollback','uninstall'].includes(action)) await this.refresh(); }catch(e){ this.toast(`Helm ${action} failed: ${human(e.body?.stderr || e.body?.error || e.message)}`); } }

...
class TaskMfe extends Base{
 constructor(){super();this.newTitle='';this.config={icon:'✅',eyebrow:'Delivery · Tasks',title:'Task Management',subtitle:'Dedicated task board backed by the task-service API. It does not render placeholder tasks.',tabs:[
  {id:'tasks',label:'Tasks',url:'/api/tasks?organizationId=default-org',columns:['Task','Status','Owner / Labels','Actions']},
  {id:'events',label:'Platform events',url:'/api/events',columns:['Event','Type','Source','Actions']},
  {id:'notifications',label:'Notifications',url:'/api/notifications/live',columns:['Notification','Status','Channel','Actions']}
 ]}}
 renderBody(m,active,rows){return `<div class="grid"><div class="metric"><div class="label">Tasks</div><div class="value">${this.tabItems('tasks').length}</div></div><div class="metric"><div class="label">Events</div><div class="value">${this.tabItems('events').length}</div></div><div class="metric"><div class="label">Notifications</div><div class="value">${this.tabItems('notifications').length}</div></div><div class="metric"><div class="label">Unavailable sources</div><div class="value">${Object.keys(this.state.errors).length}</div></div></div><div class="split"><div class="card"><h3>Create task</h3><input class="search" data-new-title placeholder="Task title from real API..." style="width:100%;margin-bottom:10px"><div class="actions"><button class="btn primary" data-action="create-task" data-tab="tasks" data-id="new">Create</button><button class="btn" data-act="refresh">Refresh</button></div></div><div class="card"><h3>Live task workflow</h3><p class="muted">Actions call task-service through the gateway. Empty columns mean the service returned no task records.</p><div class="badgeRow"><span class="pill">todo</span><span class="pill">in-progress</span><span class="pill">done</span><span class="pill">blocked</span></div></div></div><div style="height:18px"></div><div class="shell"><div class="side"><div class="sectionTitle">Task sections</div>${(this.config.tabs||[]).map(t=>`<button class="navbtn ${this.state.active===t.id?'active':''}" data-tab="${esc(t.id)}"><span>${esc(t.label)}</span><span class="pill">${this.tabItems(t.id).length}</span></button>`).join('')}</div><div class="content"><div class="toolbar"><h2>${esc(active.label||'Tasks')}</h2><input class="search" data-search placeholder="Filter tasks..."></div>${this.renderTable(active,rows)}</div></div>`;}
 renderRow(active,x){return `<tr><td><div class="rowTitle">${esc(x.raw.title||x.raw.name||x.name||short(x.id))}</div><div class="muted">${esc(x.id||x.raw.id||'')}</div></td><td><span class="status ${statusClass(x.status||x.raw.status)}">${esc(x.status||x.raw.status||'-')}</span></td><td><div>${esc(x.raw.assignee||x.raw.owner||x.raw.organizationId||'')}</div><div class="muted">${esc((x.raw.labels||x.raw.tags||[]).toString())}</div></td><td><div class="actions">${this.renderActions(active,x)}</div></td></tr>`;}
 renderActions(active,x){const id=esc(x.id||x.raw.id||x.name); if(active.id==='tasks') return `<button class="btn small ok" data-action="task-start" data-tab="tasks" data-id="${id}">Start</button><button class="btn small ok" data-action="task-done" data-tab="tasks" data-id="${id}">Done</button><button class="btn small warn" data-action="task-block" data-tab="tasks" data-id="${id}">Block</button><button class="btn small danger" data-action="task-delete" data-tab="tasks" data-id="${id}">Delete</button>`; return `<button class="btn small" data-action="raw" data-tab="${active.id}" data-id="${id}">Raw</button>`;}
 async handleAction(action,id,tab){try{
  if(action==='create-task'){const title=this.shadowRoot.querySelector('[data-new-title]')?.value?.trim(); if(!title){this.toast('Task title required'); return;} await this.request('/api/tasks',{method:'POST',body:JSON.stringify({title,organizationId:'default-org',status:'todo',priority:'medium'})}); this.toast('Task created'); await this.refresh(); return;}
  const status={ 'task-start':'in-progress','task-done':'done','task-block':'blocked'}[action];
  if(status){ await this.request(`/api/tasks/${encodeURIComponent(id)}`,{method:'PATCH',body:JSON.stringify({status})}).catch(async()=>{await this.request(`/api/tasks/${encodeURIComponent(id)}`,{method:'PUT',body:JSON.stringify({status})})}); this.toast(`Task moved to ${status}`); await this.refresh(); return; }
  if(action==='task-delete'){ if(!confirm(`Delete task ${id}?`)) return; await this.request(`/api/tasks/${encodeURIComponent(id)}`,{method:'DELETE'}); this.toast('Task deleted'); await this.refresh(); return;}
  return super.handleAction(action,id,tab);
 }catch(e){this.toast(`${action} failed: ${e.body?.error||e.message||e}`)}}
}
if(!customElements.get('nebulaops-mfe-task-management')) customElements.define('nebulaops-mfe-task-management',TaskMfe);
})();

#!/usr/bin/env bash
# NebulaOps v22.4 — Deploy fixed Keycloak theme
# Run from the nebulaops-v22.4 project root:
...
docker compose -p nebulaops-v22-4 restart keycloak
...
ERRORS=$(docker compose -p nebulaops-v22-4 logs keycloak --tail=80 2>/dev/null | grep -iE 'freemarker|ParseException' || true)
if [ -z "$ERRORS" ]; then

#!/usr/bin/env bash
# NebulaOps v22.3 — Deploy fixed Keycloak theme
# Run from the nebulaops-v22.3 project root:
...
docker compose -p nebulaops-v22-3 restart keycloak
...
ERRORS=$(docker compose -p nebulaops-v22-3 logs keycloak --tail=80 2>/dev/null | grep -iE 'freemarker|ParseException' || true)
if [ -z "$ERRORS" ]; then

#!/usr/bin/env bash
# NebulaOps v24.1 — Deploy fixed Keycloak theme
# Run from the nebulaops-v24.1 project root:
...
docker compose -p nebulaops-v24-1 restart keycloak
...
ERRORS=$(docker compose -p nebulaops-v24-1 logs keycloak --tail=80 2>/dev/null | grep -iE 'freemarker|ParseException' || true)
if [ -z "$ERRORS" ]; then

#!/usr/bin/env bash
# NebulaOps v23.1 — Deploy fixed Keycloak theme
# Run from the nebulaops-v23.1 project root:
...
docker compose -p nebulaops-v23-1 restart keycloak
...
ERRORS=$(docker compose -p nebulaops-v23-1 logs keycloak --tail=80 2>/dev/null | grep -iE 'freemarker|ParseException' || true)
if [ -z "$ERRORS" ]; then

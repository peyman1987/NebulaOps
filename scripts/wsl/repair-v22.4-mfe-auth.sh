...
log_step "Rebuilding all frontend/MFE runtime containers with the v22.4 auth bridge"
"$ROOT_DIR/scripts/wsl/repair-v22.4-frontend-remotes.sh"

check_auth_bridge_on_port() {
  local port="$1" body token
  body="$(curl -fsS --max-time 8 "http://localhost:${port}/remoteEntry.js?v=v22.4.6-live-real-data" 2>/dev/null || true)"
  if [ -z "$body" ]; then

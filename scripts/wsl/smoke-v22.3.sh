#!/usr/bin/env bash
# NebulaOps v22.3 smoke test: validates the new v22.3 API surface through the gateway.
set -uo pipefail
...
echo "[NebulaOps] v22.3 gateway smoke test"
echo "Gateway: $BASE"
...
check_post "Release promote"           "/api/releases/rel-22.3-001/promote" '{"targetEnvironment":"local"}'
check_post "Release rollback"          "/api/releases/rel-22.3-001/rollback" '{"revision":"previous"}'
...
check_post "Platform event record"     "/api/events" '{"type":"SMOKE_TEST","source":"smoke-v22.3","actor":"local","correlationId":"smoke-v22.3"}'

#!/usr/bin/env bash
# v24.1 — Fast static verification before runtime start.
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/lib/common.sh"
cd "$ROOT_DIR"

log_step "NebulaOps v24.1 static preflight"
...
# verify-remotes.mjs validates generated remoteEntry.js bundles. Keep node --check
# focused on source tools so preflight remains fast even with large generated MFE bundles.
for f in frontend/tools/*.mjs frontend/tools/*.js; do
...
if command -v docker >/dev/null 2>&1 && timeout 8s docker info >/dev/null 2>&1; then
...
log_ok "NebulaOps v24.1 preflight completed"

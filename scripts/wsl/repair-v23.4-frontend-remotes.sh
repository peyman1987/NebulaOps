#!/usr/bin/env bash
# NebulaOps v23.4 frontend/MFE runtime repair.
...
image_for_service() {
  ...
  if [ "$svc" = "frontend" ]; then
    printf 'nebulaops-v23-4-frontend-shell:latest\n'
  else
    printf 'nebulaops-v23-4-%s:latest\n' "$svc"
  fi
}
...
sync_frontend_runtime_container_from_local_dist() {
  ...
  # Remove every old conf.d file before installing the v23.4 config. Some previous
  ...
  log_ok "Running frontend container has been synced from local v23.4 dist artifacts"
}
...
sync_mfe_runtime_containers_from_local_dist() {
  ...
  log_ok "All running MFE containers have been synced from local v23.4 dist artifacts"
}
...
write_frontend_runtime_marker() {
  ...
  cid="$(frontend_container_id)"
  [ -n "$cid" ] || return 1
  marker="nebulaops-v23.4-frontend-${cid}-$(date +%s)"
...
assert_local_remote_is_classic() {
  ...
  if ! grep -Eq 'NebulaOps v23.4 auth bridge|nebulaopsAuthBridge' "$file"; then
    if ! grep -Eq 'NebulaOps v23.4 auth bridge|nebulaopsAuthBridge' "frontend/remotes/${slug}/dist/browser/nebulaops-auth-bridge.js" 2>/dev/null; then
...
if ! wait_frontend_shell 180; then
  frontend_shell_diagnostics
  log_err "Frontend shell is not reachable from WSL. Fix the container/port issue above, then rerun: ./scripts/wsl/repair-v23.4-frontend-remotes.sh"
  exit 1
fi
assert_public_url_targets_frontend_container || {
  log_err "The public host is not connected to the rebuilt frontend container. Stop any other process/container on port ${NEBULAOPS_HTTP_PORT:-80}, then rerun: ./scripts/wsl/repair-v23.4-frontend-remotes.sh"
  exit 1
}
failed=0
for slug in "${remote_slugs[@]}"; do
...
log_ok "All NebulaOps v23.4 MFE runtime bundles are rebuilt and shell-compatible"
log_info "Open ${NEBULAOPS_PUBLIC_URL}/?v=v23.4.0-live-real-data and press Ctrl+Shift+R. If Chrome still shows old JavaScript, clear site data for nebulaops.localhost."

#!/usr/bin/env bash
# Rebuilds the NebulaOps v22.3 shell and ALL MFE nginx runtime images after remoteEntry/index fixes.
...
images=(
  nebulaops-v22-3-frontend-shell:latest
  nebulaops-v22-3-mfe-docker-desktop:latest
  nebulaops-v22-3-mfe-openlens-kubernetes:latest
  nebulaops-v22-3-mfe-task-management:latest
  nebulaops-v22-3-mfe-observability:latest
  nebulaops-v22-3-mfe-cicd-gitops:latest
  nebulaops-v22-3-mfe-terraform-studio:latest
  nebulaops-v22-3-mfe-devsecops:latest
  nebulaops-v22-3-mfe-ai-ops:latest
  nebulaops-v22-3-mfe-finops-cost:latest
  nebulaops-v22-3-mfe-infra-hub:latest
  nebulaops-v22-3-mfe-release-center:latest
  nebulaops-v22-3-mfe-policy-center:latest
  nebulaops-v22-3-mfe-notification-center:latest
)

remote_slugs=(docker-desktop openlens-kubernetes task-management observability cicd-gitops terraform-studio devsecops ai-ops finops-cost infra-hub release-center policy-center notification-center)

assert_served_remote_is_classic() {
  local slug="$1" body
  body="$(curl -fsS --max-time 8 "${NEBULAOPS_PUBLIC_URL}/remotes/${slug}/remoteEntry.js?v=v22.3.6-live-real-data" 2>/dev/null || true)"
  if [ -z "$body" ]; then
...
log_ok "All v22.3 shell/MFE containers were rebuilt from live-only endpoint remoteEntry artifacts"
log_info "Open ${NEBULAOPS_PUBLIC_URL}/?v=v22.3.6-live-real-data and press Ctrl+Shift+R. If Chrome still shows old JS, clear site data for nebulaops.localhost."

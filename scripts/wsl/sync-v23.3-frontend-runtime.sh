#!/usr/bin/env bash
# NebulaOps v23.3 runtime overlay sync.
...
assert_local_remote_is_classic() {
  ...
  if ! grep -Eq 'NebulaOps v23.3 auth bridge|nebulaopsAuthBridge' "$file"; then
    if ! grep -Eq 'NebulaOps v23.3 auth bridge|nebulaopsAuthBridge' "$ROOT_DIR/frontend/remotes/${slug}/dist/browser/nebulaops-auth-bridge.js" 2>/dev/null; then
...
sync_frontend_runtime_container_from_local_dist() {
  ...
  log_ok "Frontend container content is now synced from local v23.3 dist artifacts"
}
...
sync_mfe_runtime_containers_from_local_dist() {
  ...
  log_ok "Running MFE containers are now synced from local v23.3 dist artifacts"
}
...
log_step "Syncing v23.3 frontend runtime from local dist"
sync_frontend_runtime_container_from_local_dist
sync_mfe_runtime_containers_from_local_dist
write_frontend_runtime_marker
...
log_ok "NebulaOps v23.3 frontend runtime is fully synced and shell-compatible"

#!/usr/bin/env bash
# v23.3 — Comprehensive platform health check with Keycloak-aware API and SSO proxy checks.
set -uo pipefail
...
check_required_service_endpoint() {
  ...
  if curl -fsS --max-time 5 "$url" >/dev/null 2>&1; then
    ...
    print_bad "$label" "service not running; repair: ./scripts/wsl/repair-v23.3-red-endpoints.sh"
  fi
}
...
check_mfe_remote() {
  ...
  if [ -z "$body" ]; then
    if [ -n "$service" ] && ! running_service "$service"; then
      print_bad "$label" "service not running; repair: ./scripts/wsl/repair-v23.3-frontend-remotes.sh"
    else
  ...
  if printf '%s' "$body" | grep -Eq '\bexport[[:space:]]+(default|\{|class|function|const|let|var)'; then
    print_bad "$label" "ESM remoteEntry served; run ./scripts/wsl/repair-v23.3-frontend-remotes.sh"
    return 0
  fi

...
log_step "Hard-resetting NebulaOps v23.3 frontend/MFE runtime"
export NEBULAOPS_FORCE_NO_CACHE_MFE_IMAGES=true
export NEBULAOPS_CACHE_BUSTER="v23.3-authbridge-$(date +%Y%m%d%H%M%S)"
...
log_info "Removing stale NebulaOps v23.3 frontend/MFE images"
docker image ls --format '{{.Repository}}:{{.Tag}}' \
  | grep -E '^nebulaops-v23-3-(frontend-shell|mfe-)' \

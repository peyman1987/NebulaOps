#!/usr/bin/env bash
# NebulaOps v22.4 live integration smoke test.
set -uo pipefail
...
echo "[NebulaOps] v22.4 LIVE integration smoke test"
echo "Gateway: $BASE"
...
check_post "Release promotion policy gate"     "/api/releases/rel-22.4-001/promote" '{"targetEnvironment":"local","budgetThreshold":75}'
check_post "Release rollback audit event"      "/api/releases/rel-22.4-001/rollback" '{"revision":"previous"}'
check_post "Policy evaluation live"            "/api/policies/evaluate" '{"target":"gateway-service","image":"nebulaops-v22-4-gateway-service:22.4.0","budgetThreshold":75}'
check_post "AI Ops live RCA"                   "/api/ai-ops/incidents/analyze" '{"affectedService":"gateway-service","namespace":"default"}'
check_post "DevSecOps image scan + audit"      "/api/devsecops/scan/image" '{"image":"nebulaops-v22-4-gateway-service:22.4.0"}'

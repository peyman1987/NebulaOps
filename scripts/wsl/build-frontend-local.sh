#!/usr/bin/env bash
# NebulaOps v23.4 local frontend builder.
...
file_has_classic_remote_contract() {
  ...
  grep -Eq 'NebulaOps v23.4 auth bridge|nebulaopsAuthBridge' "$file" || return 1
...
if [ "${NEBULAOPS_FORCE_FRONTEND_DIST_BUILD:-false}" != "true" ] && existing_dist_is_valid; then
  echo "[NebulaOps] Existing v23.4 frontend/MFE dist artifacts are valid; skipping npm rebuild."

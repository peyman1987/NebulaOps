#!/usr/bin/env bash
# Repair only the v22.3 endpoints that can appear red after an upgrade from an earlier package:
...
log_step "Releasing v22.3 red endpoint ports"
for port in 8097; do
  release_port_if_nebulaops "$port" "v22.3 red endpoint repair"
done
...
log_step "Recreating v22.3 extended micro frontends"
dc build \
  mfe-infra-hub \
  mfe-release-center \
  mfe-policy-center \
...
log_ok "v22.3 red endpoints repaired"

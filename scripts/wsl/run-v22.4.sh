#!/usr/bin/env bash
# NebulaOps v22.4 one-command launcher.
...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/wsl/run-v22.4.sh [options]
...
Default behavior:
  ...
  5. run health and v22.4 live smoke checks
...
Examples:
  ./scripts/wsl/run-v22.4.sh
  ./scripts/wsl/run-v22.4.sh --with-gitlab
  ./scripts/wsl/run-v22.4.sh --skip-build
USAGE
}
...
banner "NebulaOps v22.4 one-command startup"
...
if [ "$SKIP_SMOKE" = false ]; then
  banner "7) Smoke test"
  if [ -x ./scripts/wsl/smoke-v22.4-live.sh ]; then
    ./scripts/wsl/smoke-v22.4-live.sh || {
      echo "[WARN] v22.4 live smoke reported failures. Check logs with ./scripts/wsl/logs.sh"
    }
  elif [ -x ./scripts/wsl/smoke-v22.4.sh ]; then
    ./scripts/wsl/smoke-v22.4.sh || {
      echo "[WARN] v22.4 smoke reported failures. Check logs with ./scripts/wsl/logs.sh"
    }
  fi
fi

banner "NebulaOps v22.4 is started"
cat <<NEXT
Useful URLs:
  Shell:        http://nebulaops.localhost
  Gateway:      http://localhost:8080/actuator/health
  Keycloak:     http://nebulaops.localhost/keycloak
  Grafana:      http://localhost:${GRAFANA_HOST_PORT:-3300}

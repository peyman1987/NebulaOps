...
log_step "Hard-resetting NebulaOps v24.1 frontend/MFE runtime"
export NEBULAOPS_FORCE_NO_CACHE_MFE_IMAGES=true
export NEBULAOPS_CACHE_BUSTER="v24.1-authbridge-$(date +%Y%m%d%H%M%S)"
...
log_info "Removing stale NebulaOps v24.1 frontend/MFE images"
docker image ls --format '{{.Repository}}:{{.Tag}}' \
  | grep -E '^nebulaops-v24-1-(frontend-shell|mfe-)' \

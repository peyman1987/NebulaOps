#!/usr/bin/env bash
# NebulaOps v22.4 frontend runtime guard.
...
for remote_dir in frontend/remotes/*; do
  ...
  if ! grep -Eq 'NebulaOps v22.4 auth bridge|nebulaopsAuthBridge' "$remote_dir/dist/browser/remoteEntry.js" 2>/dev/null; then
...
if [ "$missing" -ne 0 ]; then
  log_err "Frontend dist artifacts are incomplete or shell-incompatible. Run: ./scripts/wsl/repair-v22.4-frontend-remotes.sh"
  exit 1
fi

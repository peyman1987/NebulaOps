#!/usr/bin/env bash
# v24.1 — release identity smoke guard for docs, package metadata, Maven, Docker and frontend runtime assets.
set -euo pipefail
source "$(dirname "${BASH_SOURCE[0]}")/lib/common.sh"
cd "$ROOT_DIR"

log_step "Checking NebulaOps v24.1 release identity alignment"
python3 scripts/validate-version-alignment-v24.1.py

log_step "Checking start-script referenced v24.1 helpers"
required=(
...
for f in "${required[@]}"; do
  [[ -f "$f" ]] || log_err "Missing v24.1 aligned file: $f"
done
...
log_ok "NebulaOps v24.1 release identity alignment verified"

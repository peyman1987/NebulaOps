#!/usr/bin/env bash
# NebulaOps v23.4 hotfix/diagnostic for stale same-origin MFE remoteEntry.js.
...
if [ -f "$ROOT_DIR/scripts/wsl/lib/common.sh" ]; then
  ...
  PROJECT_NAME="${COMPOSE_PROJECT_NAME:-nebulaops-v23-4}"
...
marker="nebulaops-v23.4-hotfix-${frontend_cid}-$(date +%s)"
...
for slug in "${remote_slugs[@]}"; do
  ...
  if ! grep -Eq 'NebulaOps v23.4 auth bridge|nebulaopsAuthBridge' "$public_file"; then
...
echo "[ok] v23.4 frontend remote routing is consistent."

#!/usr/bin/env bash
# NebulaOps v23.2 hotfix/diagnostic for stale same-origin MFE remoteEntry.js.
...
marker="nebulaops-v23.2-hotfix-${frontend_cid}-$(date +%s)"
...
for slug in "${remote_slugs[@]}"; do
  ...
  if ! grep -Eq 'NebulaOps v23.2 auth bridge|nebulaopsAuthBridge' "$public_file"; then
...
echo "[ok] v23.2 frontend remote routing is consistent."

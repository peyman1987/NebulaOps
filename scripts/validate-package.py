...
try:
    ...
    if "NebulaOps v23.4 extension theme override" not in (ROOT / "extensions/apiforge/src/main/resources/static/css/app.css").read_text(encoding="utf-8"):
...
# v23.4 release hygiene: visible/runtime version identifiers must be aligned.
try:
    ignored = {
    ...
    errors.append(f"v23.4 release hygiene validation failed: {exc}")

# v23.4 release manifest checks: prevent add-on manifests from keeping old application versions.
try:
    release_manifest_checks = {
        "infrastructure/helm/addons/nebulaops-spring-mvc-addon/Chart.yaml": ["appVersion: \"23.4.0\""],
        "infrastructure/kubernetes/addons/spring-mvc-service.yaml": ["app.kubernetes.io/version: \"23.4.0\""],
    }
    ...
    errors.append(f"v23.4 release manifest validation failed: {exc}")
...
print("v23.4 runtime stability and release hygiene checks aligned")

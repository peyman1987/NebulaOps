...
try:
    ...
    if "NebulaOps v24.1 extension theme override" not in (ROOT / "extensions/apiforge/src/main/resources/static/css/app.css").read_text(encoding="utf-8"):
...
# v24.1 release hygiene: visible/runtime version identifiers must be aligned.
try:
    ignored = {
    ...
    errors.append(f"v24.1 release hygiene validation failed: {exc}")

# v24.1 release manifest checks: prevent add-on manifests from keeping old application versions.
try:
    release_manifest_checks = {
        "infrastructure/helm/addons/nebulaops-spring-mvc-addon/Chart.yaml": ["appVersion: \"24.1.0\""],
        "infrastructure/kubernetes/addons/spring-mvc-service.yaml": ["app.kubernetes.io/version: \"24.1.0\""],
    }
    ...
    errors.append(f"v24.1 release manifest validation failed: {exc}")
...
print("v24.1 runtime stability and release hygiene checks aligned")

...
TASK_RESPONSE=$(curl -fsS -H 'Content-Type: application/json'   -d '{"organizationId":"demo-org","projectId":"portfolio","title":"Smoke test RabbitMQ event","description":"Created by smoke-test.sh","priority":"HIGH","labels":["smoke","rabbitmq","mongodb"]}'   "$BASE/api/tasks")
echo "$TASK_RESPONSE"
echo "Listing tasks..."

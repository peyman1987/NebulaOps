...
The v24.1 local runtime uses a single browser-facing origin:
...
## v24.1 Foundation, Build and Frontend Architecture Upgrade

The v24.1 baseline adds three operational improvements:

- release alignment is guarded by `scripts/validate-version-alignment-v24.1.py`;
- frontend builds use one root dependency install plus selective MFE rebuild cache;
- frontend architecture contracts are centralized in `frontend/libs` for API calls, source state, MFE bootstrap and UI Kit v2.

Fast local mode and full mode are both available:
...
`start.sh --rebuild` remains full by default for backward compatibility. `--core` starts the lighter profile for local development.

## v24.1 release identity
...
NebulaOps v24.1 now includes a dedicated **Platform Catalog & Service Registry** section at:
...
NebulaOps v24.1 now includes **Incident Command Center** at:
...
Current documentation is aligned with the v24.1 same-origin runtime:
...
The v24.1 observability console is live-only. It reads service health, Prometheus, Loki, Tempo, RabbitMQ, task, notification and audit endpoints through `/api/observability/**`. It does not render seeded, sample or mock records; empty tables mean the runtime source returned no rows or was unavailable.


## Progressive Delivery Center

NebulaOps v24.1 includes a Progressive Delivery Center for live Argo Rollouts and Argo CD operations. It reads only runtime data from Kubernetes, Argo Rollouts and Argo CD through `/api/progressive-delivery/**`; empty states indicate no live records or an unreachable runtime source.

### APIForge on Kubernetes

NebulaOps v24.1 includes APIForge in the App Bar. The source is packaged as an extension under `extensions/apiforge/`. APIForge is deployed directly to Kubernetes and exposed on the local NodePort `31110`:
...
## NebulaOps v24.1 Extensions

NebulaOps v24.1 now keeps only APIForge as Kubernetes-hosted extension under `extensions/`, styled with the same NebulaOps dark/glass UI language and exposed in the App Bar:
...
## v24.1 real-extension policy

The v24.1 extensions do not ship operational records as artificial seed data. Runtime screens call Spring Boot MVC endpoints and render only live responses from Kubernetes, mounted runbook/SLO files or configured service APIs. Missing integrations are reported as `NOT_CONFIGURED`, `DEGRADED` or `UNAVAILABLE`.


## APIForge UI-controlled extension

NebulaOps v24.1 keeps only APIForge under `extensions/`. The core platform starts first; APIForge can then be started from the UI:
...
## v24.1 UI-controlled extensions
...
Version 24.1 introduces a dedicated Operational Issues Dashboard for live runtime troubleshooting. The shell now includes `/remotes/operational-issues/`, backed by the gateway endpoints `/api/platform/issues`, `/api/platform/issues/summary`, and `/api/platform/issues/{id}/evidence`.
...
## v24.1 Extension Async Control Plane

NebulaOps v24.1 starts and restarts installed extensions through an asynchronous control-plane model. `POST /api/extensions/{slug}/start` returns `202 Accepted` with an `operationId`; the UI polls `/api/extensions/operations/{operationId}` and reads `/api/extensions/{slug}/events` for live phase updates.
...
```bash
./scripts/wsl/smoke-extension-async-control-plane-v24.1.sh
```

To verify the full real deployment flow:

```bash
NEBULAOPS_RUN_ASYNC_EXTENSION_START_SMOKE=1 ./scripts/wsl/smoke-extension-async-control-plane-v24.1.sh
```

## v24.1 Diagnostics Bundle Export

NebulaOps v24.1 includes a live support/debug export for runtime troubleshooting:
...
## v24.1 E2E Runtime Smoke Tests

NebulaOps v24.1 now includes E2E smoke tests for runtime regressions:

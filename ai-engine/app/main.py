...
def call_anthropic(req: AnalyzeRequest, api_key: str) -> dict[str, Any]:
    payload = {
        ...
        "temperature": 0.1,
        "system": (
            "You are NebulaOps v24.1 AI Engine. Analyze only the runtime evidence supplied by the caller. "
...
def fallback_analysis(req: AnalyzeRequest, reason: str) -> dict[str, Any]:
    ...
    return normalize_response({
        "incidentId": "AIOPS-24-1-" + datetime.now(timezone.utc).strftime("%H%M%S"),
...
def normalize_response(payload: dict[str, Any], req: AnalyzeRequest) -> dict[str, Any]:
    ...
    payload.setdefault("incidentId", "AIOPS-24-1-" + datetime.now(timezone.utc).strftime("%H%M%S"))
